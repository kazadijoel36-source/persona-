...
(function () {
  ...
  function initDashboard() {
    ...
    Promise.all([PersonaAPI.getProfile(), PersonaAPI.getWordBalance()]).then(
      ([profile, balance]) => {
        ...
        const pct = Math.min(100, (balance.used / balance.cap) * 100);
        document.querySelector("#usageFill").style.width = pct + "%";
        document.querySelector("#usageUsed").textContent = balance.used.toLocaleString();
        document.querySelector("#usageCap").textContent = balance.cap.toLocaleString();
  ...
  // expose chart helpers in case a page wants them inline
